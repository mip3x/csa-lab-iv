...
|-------------|--------------------|-------------------|-----------------------------------------------------------------------------------------------------------------------|
|   **add**   | `add rd, rs1, rs2` | `rd <- rs1 + rs2` | Сложить значения в регистрах `<rs1>` и `<rs2>`, результат сохранить в `<rd>`                                          |
|             | `add rd, rs1, imm` | `rd <- rs1 + imm` | Сложить значение регистра `<rs1>` и константу `<imm>`, результат сохранить в `<rd>`                                   |
|   **sub**   | `sub rd, rs1, rs2` | `rd <- rs1 – rs2` | Вычесть значение регистра `<rs2>` из регистра `<rs1>`, результат сохранить в `<rd>`                                   |
|             | `sub rd, rs1, imm` | `rd <- rs1 – imm` | Вычесть константу `<imm>` из значения регистра `<rs1>`, результат сохранить в `<rd>`                                  |
|   **mul**   | `mul rd, rs1, rs2` | `rd <- rs1 * rs2` | Перемножить значения регистров `<rs1>` и `<rs2>`, результат сохранить в `<rd>`                                        |
|   **div**   | `div rd, rs1, rs2` | `rd <- rs1 / rs2` | Поделить значение регистра `<rs1>` на значение регистра `<rs2>` (целочисленное деление), результат сохранить в `<rd>` |
|   **and**   | `and rd, rs1, rs2` | `rd <- rs1 & rs2` | Выполнить побитовое `И` между регистрами `<rs1>` и `<rs2>`, результат сохранить в `<rd>`                              |
|             | `and rd, rs1, imm` | `rd <- rs1 & imm` | Выполнить побитовое `И` между значением регистра `<rs1>` и константой `<imm>`, результат сохранить в `<rd>`           |
|   **cmp**   | `cmp rs1, rs2`     | `SR <- NZVC`      | Сравнивает значения `<rs1>` и `<rs2>`, выставить флаги процессора `NZVC`, результат сохранить в регистр `SR`          |
