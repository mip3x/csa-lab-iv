...
Общее количество регистров - **16**. Регистр является **32-битным**, если не указано иное. Программист не может явно взаимодействовать ни с одним из регистров
...
- `IR` (`Instruction Register`): хранит исполняемую команду; размер - `16 бит` (`2 байта`)
...
| Мнемоника   | Синтаксис          | Семантика         | Описание                                                                                                              |
|-------------|--------------------|-------------------|-----------------------------------------------------------------------------------------------------------------------|
|   **add**   | `add rd, rs1, rs2` | `rd <- rs1 + rs2` | Сложить значения в регистрах<br/> `<rs1>` и `<rs2>`, результат сохранить в `<rd>`                                          |
|             | `add rd, rs1, imm` | `rd <- rs1 + imm` | Сложить значение регистра `<rs1>` и<br/> константу `<imm>`, результат сохранить в `<rd>`                                   |
|   **sub**   | `sub rd, rs1, rs2` | `rd <- rs1 – rs2` | Вычесть значение регистра `<rs2>` из<br/> регистра `<rs1>`, результат сохранить в `<rd>`                                   |
|             | `sub rd, rs1, imm` | `rd <- rs1 – imm` | Вычесть константу `<imm>` из<br/> значения регистра `<rs1>`, результат сохранить в `<rd>`                                  |
|   **mul**   | `mul rd, rs1, rs2` | `rd <- rs1 * rs2` | Перемножить значения регистров<br/> `<rs1>` и `<rs2>`, результат сохранить в `<rd>`                                        |
|   **div**   | `div rd, rs1, rs2` | `rd <- rs1 / rs2` | Поделить значение регистра<br/> `<rs1>` на значение регистра `<rs2>` (целочисленное деление), результат сохранить в `<rd>` |
|   **and**   | `and rd, rs1, rs2` | `rd <- rs1 & rs2` | Выполнить побитовое `И` между<br/> регистрами `<rs1>` и `<rs2>`, результат сохранить в `<rd>`                              |
|             | `and rd, rs1, imm` | `rd <- rs1 & imm` | Выполнить побитовое `И` между<br/> значением регистра `<rs1>` и константой `<imm>`, результат сохранить в `<rd>`           |
|   **cmp**   | `cmp rs1, rs2`     | `SR <- NZVC`      | Сравнивает значения `<rs1>` и `<rs2>`,<br/> выставить флаги процессора `NZVC`, результат сохранить в регистр `SR`          |
