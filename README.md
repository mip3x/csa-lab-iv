...
|-------------|--------------------|-------------------|----------|
|   **add**   | `add rd, rs1, rs2` | `rd <- rs1 + rs2` | Складывает значения в регистрах `<rs1>` и `<rs2>`, результат сохраняет в `<rd>` |
|             | `add rd, rs1, imm` | `rd <- rs1 + imm` | Складывает значение регистра `<rs1>` и константу `<imm>`, результат сохраняет в `<rd>` |
|   **sub**   | `sub rd, rs1, rs2` | `rd <- rs1 – rs2` | Вычитает значение регистра `<rs2>` из регистра `<rs1>`, результат сохраняет в `<rd>` |
|             | `sub rd, rs1, imm` | `rd <- rs1 – imm` | Вычитает константу `<imm>` из значения регистра `<rs1>`, результат сохраняет в `<rd>` |
|   **mul**   | `mul rd, rs1, rs2` | `rd <- rs1 * rs2` | Умножает значения регистров `<rs1>` и `<rs2>`, результат сохраняет в `<rd>` |
|   **div**   | `div rd, rs1, rs2` | `rd <- rs1 / rs2` | Делит значение регистра `<rs1>` на значение регистра `<rs2>` (целочисленное деление), результат сохраняет в `<rd>` |
|   **and**   | `and rd, rs1, rs2` | `rd <- rs1 & rs2` | Побитовое "И" между регистрами `<rs1>` и `<rs2>`, результат сохраняет в `<rd>` |
|             | `and rd, rs1, imm` | `rd <- rs1 & imm` | Побитовое "И" между значением регистра `<rs1>` и константой `<imm>`, результат сохраняет в `<rd>` |
|   **cmp**   | `cmp rs1, rs2`     | `SR <- NZVC` | Сравнивает значения `<rs1>` и `<rs2>`, выставляет флаги процессора `N` (negative), `Z` (zero), `V` (overflow), `C` (carry), результат сохраняется в регистр `SR` |
